...
public class StressButtonService extends IntentService {
	...
	private void storeResponse(String surveyId, String surveyTitle, String launchTime, List<Prompt> prompts) {
		...
		if (loc != null) {
		//	dbHelper.addResponseRow(campaign, username, date, time, timezone, SurveyGeotagService.LOCATION_VALID, loc.getLatitude(), loc.getLongitude(), loc.getProvider(), loc.getAccuracy(), loc.getTime(), surveyId, surveyLaunchContext, response);
		} else {
			//dbHelper.addResponseRowWithoutLocation(campaign, username, date, time, timezone, surveyId, surveyLaunchContext, response);
		}
	}
}
